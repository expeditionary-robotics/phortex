...
code_test = True
experiment_name = f"local_phumes_iterativeplans_seed{np.random.randint(low=0, high=1000)}"
print("Experiment Name: ", experiment_name)

# Set iteration parameters
if code_test:
    sample_iter = 5  # number of samples to search over
    burn = 0  # number of burn-in samples
    plan_iter = 15  # planning iterations
    samp_dist = 1.0  # distance between samples (in meters)
    time_resolution = 1 * 3600  # time resolution (in seconds)
    duration = 3 * 3600  # total mission time (in seconds)
    num_snaps = 3
    sampling_height = 120.
    sim_traj = "naive"  # or phortex

else:
    sample_iter = 650  # number of samples to search over
    burn = 150  # number of burn-in samples
    plan_iter = 15  # planning iterations
    samp_dist = 1.0  # distance between samples (in meters)
...
for st in snap_times:
    ...
    env_snapshot = env.get_snapshot(t=st, z=[sampling_height], from_cache=False)
    ...
    # plot learned model
    mod_snapshot = mtt.get_snapshot(t=st, z=[sampling_height], from_cache=False)
...
exp_name = f"{experiment_name}_training"
mtt.experiment_name = exp_name
altitude = sampling_height  # flight altitude (in meters)

if sim_traj is "naive":
    planners = []
    traj_opt_saver = None
...
simulator.simulate(times,
                   experiment_name=f"{experiment_name}_training",
...
# Log information
experiment_dict = {"experiment_iteration": 0,
