...
def plot_trajectory_map(trajectory, extent, site="ridge", show_vis=False, save_file=None):
    """Plots bathy, sites, and other relevant information with trajectory object."""
    coords = np.asarray(trajectory.uniformly_sample(0.5))
    if extent.origin[0] == 0.0:
        #  Local coordinates
        xcoords = coords.T[1, :]
        ycoords = coords.T[2, :]
        traj_fig = go.Scatter(x=xcoords, y=ycoords)
        fig = go.Figure(data=traj_fig, layout=dict(width=400, height=400))
        fig.update_layout(xaxis_range=[extent.xmin, extent.xmax],
                          yaxis_range=[extent.ymin, extent.ymax])
    else:
        # Global coordinates
...
def visualize_and_save_traj(trajectory, extent, traj_name="temp_traj",
    ...
    Args:
        ...
        generate_sentry_file (bool): if True, generate a global Sentry
            trajectory file
    """
    with open(f"{traj_name}.pkl", "wb") as fh:
        pickle.dump(trajectory, fh)

    plot_trajectory_map(trajectory, extent, save_file=f"{traj_name}.png")

    # If using global coordinates, generate sentry files
    if generate_sentry_file:
